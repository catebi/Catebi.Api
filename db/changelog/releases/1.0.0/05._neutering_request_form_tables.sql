...
drop table if exists ctb.neutering_sponsor_type;
...
create table ctb.neutering_request(
    ...
    need_carrier bool,    
    need_catcher bool,    
    neutering_sponsor_type_id int not null, 
    clinic_id int,
...
comment on column ctb.neutering_request.need_carrier  is 'Нужны ли переноски';
comment on column ctb.neutering_request.need_catcher  is 'Нужны ли котоловки';
comment on column ctb.neutering_request.neutering_sponsor_type_id  is 'Платит ли заполнитель за стерилизацию'; 
comment on column ctb.neutering_request.clinic_id  is 'id вет.клиники, где будет стерилизация';
comment on column ctb.neutering_request.cat_count_foster  is 'Скольких кошек пользователь может взять себе на передержку после стерилизации';
comment on column ctb.neutering_request.cat_returning_by_user  is 'Организация доставки обратно'; 
...
create table ctb.neutering_sponsor_type(--!!! Primary filling 'плачу полностью', 'плачу частично', 'за счёт Catebi'
    neutering_sponsor_type_id serial primary key,
    option_text text unique not null
);

comment on table ctb.neutering_sponsor_type is 'Справочник к полю анкеты "Платит ли заполнитель анкеты за стерилизацию"';
comment on column ctb.neutering_sponsor_type.neutering_sponsor_type_id is 'ID';
comment on column ctb.neutering_sponsor_type.option_text is 'Текст варианта';
...
alter table ctb.neutering_request
    add foreign key (neutering_sponsor_type_id)
        references ctb.neutering_sponsor_type(neutering_sponsor_type_id);
alter table ctb.neutering_request

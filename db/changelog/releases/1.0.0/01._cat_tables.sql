...
drop table if exists ctb.cat_cat_tag;
...
-- cat_cat_tag table
create table ctb.cat_cat_tag (
    cat_cat_tag_id serial PRIMARY KEY,
    cat_id integer NOT NULL,
...
comment on table ctb.cat_cat_tag is 'Словарь для связи кошек и тегов';
comment on column ctb.cat_cat_tag.cat_cat_tag_id is 'Id соотношения';
comment on column ctb.cat_cat_tag.cat_id is 'Id кошки';
comment on column ctb.cat_cat_tag.cat_tag_id is 'Id тега';
...
--FK in cat_cat_tag table
alter table ctb.cat_cat_tag
    add FOREIGN KEY (cat_id)
		references ctb.cat(cat_id);
alter table ctb.cat_cat_tag
